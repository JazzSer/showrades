...
> **Status note (2026-08-03):** This is research/reference material, not a locked spec. The launch build now uses 7 categories (Animals, Food, Jobs, Sports, Vehicles, Everyday Stuff, Nature — see `2. Backlog & Status.md`; Fruits and Veggies were merged into Food, and Vehicles/Everyday Stuff/Nature were added from this document's category database, both on 2026-08-03). Word lists for the three newly-added categories were adapted, not copied verbatim — items without a clear, unambiguous single emoji (e.g. Submarine, Waterfall, River, Blanket, Pillow) were dropped for the age 5–8 target audience. TV & Movies and Actions remain unadded. The 8-category database and "Secret Clue" / "Educational Twist" concepts below are the basis for the **sneaky-educational expansion direction** described in `1. Vision & Goals.md` §5 — to be added **on top of** the current 7 categories, not as a replacement. Treat the per-item content below as worked examples to refine from, not final copy.
